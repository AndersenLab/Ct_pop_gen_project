...
target_lineage<-c("Af","LAC")
target_geo<-"Africa"

target_isotype<-geo_and_lineage_Af %>% 
...
# dev.off()
# pdf(paste0("Ct_",target_isotype,"_concordance_heatmap_3.pdf"), width = 7, height = 6)
# 
# 
...
isotype_anno <- geo_info_raw %>%
  ...
  dplyr::left_join(
    geo_and_lineage_raw %>% dplyr::select(isotype, lineage),
    by = "isotype"
  ) %>%
  dplyr::mutate(
    island = dplyr::case_when(
      isotype %in% c("JU1374", "JU1373", "JU1800") ~ "Reunion",
      isotype %in% c("NIC1534", "NIC1531", "NIC1526") ~ "Nosy Boraha",
      isotype %in% c("JU3170") ~ "Sao Tome",
      TRUE ~ "Cape Verde"
    )
  ) %>%
  tibble::column_to_rownames(var = "isotype") %>%
  dplyr::select(island, lineage, lat, long)

isotype_anno$island <- as.factor(isotype_anno$island)

col_ordered_island_target  <- isotype_anno[sorted_cols, "island", drop = FALSE]
col_ordered_lineage_target <- isotype_anno[sorted_cols, "lineage", drop = FALSE]
col_ordered_lat_target     <- as.numeric(isotype_anno[sorted_cols, "lat"])
col_ordered_long_target    <- as.numeric(isotype_anno[sorted_cols, "long"])
...
filter_island.colours_target <- island.colours[
  names(island.colours) %in% unique(col_ordered_island_target$island)
]

lat_col_fun <- circlize::colorRamp2(
  c(
    min(col_ordered_lat_target),
    mean(c(min(col_ordered_lat_target), max(col_ordered_lat_target)), na.rm = TRUE),
    max(col_ordered_lat_target)
  ),
  c("#DFF5E1", "#4CAF50", "#1B5E20")
...
long_col_fun <- circlize::colorRamp2(
  c(
    min(col_ordered_long_target),
    mean(c(min(col_ordered_long_target), max(col_ordered_long_target)), na.rm = TRUE),
    max(col_ordered_long_target)
  ),
  c("whitesmoke", "#fa9752", "#67000d")
...
col_ha_target <- HeatmapAnnotation(
  Group  = col_ordered_lineage_target$lineage,
  Island = col_ordered_island_target$island,
  Lat    = col_ordered_lat_target,
  Long   = col_ordered_long_target,
  col = list(
    Group  = filter_lineage.colours_target,
    Island = filter_island.colours_target,
    Lat    = lat_col_fun,
    Long   = long_col_fun
  ),
  show_annotation_name = TRUE,
...
table(col_ordered_island_target$island)
...
dev.off()
pdf(paste0("../../plots/FigureS18_Ct_Af_island_concordance_heatmap_4.pdf"), width = 7, height = 6)
