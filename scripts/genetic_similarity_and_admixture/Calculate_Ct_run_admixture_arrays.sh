...
#SBATCH -N 1
#SBATCH -n 4
#SBATCH --output=Calculate_Ct_run_admixture_arrays_%A_%a.oe
#SBATCH --job-name="CtAdm"
#SBATCH --array=1-240%150
...
admixture --cv=10 -s "$current_seed" \
          ...
          $current_pop \
          -j4 | tee "${out_folder}log_${current_pop}_${current_seed}.out"
