...
gtcheck_raw <- readr::read_tsv("../../processed_data/gt_check_only_HDRs/gtcheck_onlyHDR.tsv",
                           comment = "#", show_col_types = FALSE) 
gt_pairs <- gtcheck_raw %>%
  dplyr::filter(INFO == "DC") %>%
  tidyr::separate(
    `Time required to process one record .. 0.000358 seconds`,
...
pdf("../../figures/raw_FigureS33_heatmap_HDRs.pdf", 
    width = 12, height = 10)
