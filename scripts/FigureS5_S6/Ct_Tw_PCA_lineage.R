...
### input GIS data
GIS_data_raw<-read.table("../../processed_data/Taiwan/Ct_TW_isotype_GIS.txt",
                            sep = '\t',header = TRUE)


GIS_data <- GIS_data_raw

EVarNoRm_ld0.9 <- data.table::fread("../../processed_data/Taiwan/EIGESTRAT/LD_0.9/NO_REMOVAL/TracyWidom_statistics_no_removal.tsv") %>%
...
cdf9no <- data.table::fread("../../processed_data/Taiwan/EIGESTRAT/LD_0.9/NO_REMOVAL/eigenstrat_no_removal.evac", skip = 1)
...
saveRDS(cdf9_7no, file = "../../processed_data/Taiwan/cdf9_7no.rds")
...
PCA_main
# ggsave("Ct_PCA_Taiwan_Lineage.pdf",
#        PCA_main,
#        width = 7.5, height = 4, units = "in")
saveRDS(PCA_main, file = "../../processed_data/Taiwan/Ct_PCA_Taiwan_Lineage.rds")
